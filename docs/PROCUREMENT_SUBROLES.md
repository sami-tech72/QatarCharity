...
3. The guard `frontend/src/app/core/guards/auth.guard.ts` allows or blocks navigation based on those bits. Because `procurementSubRoles` is an open array of strings, you can invent new sub-role names in backend claims without changing the frontend—only the permissions determine access.
