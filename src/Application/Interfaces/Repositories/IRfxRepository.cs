...
{
    Task<int> CountSupplierBidsAsync(string? search, string? submittedByUserId = null, string? evaluationStatus = null, bool excludeWithContract = false);
    Task<IReadOnlyList<(SupplierBid Bid, RfxEntity Rfx)>> GetSupplierBidsAsync(string? search, string? submittedByUserId, int pageNumber, int pageSize, string? evaluationStatus = null, bool excludeWithContract = false);
